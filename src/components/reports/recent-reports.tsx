...
import { RecentReport, downloadReportFile } from "@/services/report.service";
...
export function RecentReportsList({ reports }: RecentReportsListProps) {
    const getFormatIcon = (format: string) => {
        switch (format) {
            ...
            default: return <FileSpreadsheet className="w-5 h-5 text-green-600" />;
        }
    };

    const handleDownloadReport = (report: RecentReport) => {
        downloadReportFile(report.name, report.format, report.date);
    };

    return (
        <div className="space-y-4 font-sans">
            <h3 className="text-[15px] font-bold text-[#1A1A1A]">Recent Reports</h3>
            <Card className="border-gray-100 shadow-sm rounded-[8px] overflow-hidden bg-white">
                    ...
                    {reports.map((report) => (
                        <div key={report.id} className="flex items-center justify-between p-4 hover:bg-gray-50/50 transition-colors group">
                            ...
                            <button
                                ...
                                className="p-2.5 text-gray-400 hover:text-primary-green hover:bg-primary-green/10 border border-gray-200 rounded-[6px] transition-all cursor-pointer"
